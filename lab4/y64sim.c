...
stat_t nexti(y64sim_t *sim)
{
    ...
    regid_t regA=REG_NONE;
	 ...
	 byte_t regflag=0;
	 if(!get_byte_val(sim->m,next_pc,&regflag)){
	 	err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
		return STAT_ADR;
	 }
	 regA=GET_REGA(regflag);
	 regB=GET_REGB(regflag);
	 if(icode==I_RRMOVQ||icode==I_RMMOVQ||icode==I_MRMOVQ||icode==I_ALU){
	 	if (!NORM_REG(regA) || !NORM_REG(regB)){
			err_print("PC = 0x%lx, Invalid register id %.2x", sim->pc, regflag);
			return STAT_INS;
		}
	 }
	 else if(icode==I_IRMOVQ){
	 	if (!NONE_REG(regA) || !NORM_REG(regB)){
			err_print("PC = 0x%lx, Invalid register id %.2x", sim->pc, regflag);
			return STAT_INS;
		}
	 }
	 else if(icode==I_PUSHQ||icode==I_POPQ){
	 	if (!NORM_REG(regA) || !NONE_REG(regB)){
			err_print("PC = 0x%lx, Invalid register id %.2x", sim->pc, regflag);
			return STAT_INS;
		}
	 }
    ...
    long_t valA, valB, valE, valM, rsp;
	 switch (icode) {
      case I_HALT: /* 0:0 */
	      return STAT_HLT;
	      break;
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
			...
			if(!set_long_val(sim->m,valB+imm,valA)){
				err_print("PC = 0x%lx, Invalid stack address %p", sim->pc, (void*)(valB+imm));
				return STAT_ADR;
			}
			sim->pc=next_pc;
			break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
			valB=get_reg_val(sim->r,regB);
			if(!get_long_val(sim->m,valB+imm,&valM)){
				err_print("PC = 0x%lx, Invalid data address %p", sim->pc, (void*)(valB + imm));
				return STAT_ADR;
			}
      ...
      case I_CALL:/* 8:x imm */
			...
			if(!set_long_val(sim->m,rsp,next_pc)){
				err_print("PC = 0x%lx, Invalid stack address %p", sim->pc, (void*)rsp);
				return STAT_ADR;
			}
			sim->pc=imm;
			break;
      case I_RET:/* 9:0 */
			...
			if (!get_long_val(sim->m, rsp, &valM)) {
				err_print("PC = 0x%lx, Invalid stack address %p", sim->pc, (void*)rsp);
				return STAT_ADR;
			}
      ...
      case I_POPQ: /* B:0 regA:F */
			rsp=get_reg_val(sim->r,REG_RSP)+8;
			set_reg_val(sim->r,REG_RSP,rsp);
			if(!get_long_val(sim->m,rsp-8,&valM)){
				 err_print("PC = 0x%lx, Invalid stack address %p", sim->pc, (void*)(rsp-8));
			}
			set_reg_val(sim->r,regA,valM);
			sim->pc=next_pc;
			break; /* unsupported now, replace it with your implementation*/
      default:
